...
//* - useRef : DOM 요소에 직접 접근하기 위한 함수, 렌더링 사이에 값을 기억하는 함수 ( 잘 사용하지 않음 )
// - useContext : 컴포넌트 트리 상에서 전역적으로 상태를 공유하고자 할 때 사용하는 함수
// - useRenducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게하여 코드 관리를 조금 더 수월하게 해줌
...
export default function HookComponent1() {
    ...
    // - useEfeect( 실행할 함수, 스코프할 상태 배열);
    // - 개발자 모드에서는 마운트 - 언마운트가 한번 실행되면서 테스트 후 다시 정상 실행 됨
